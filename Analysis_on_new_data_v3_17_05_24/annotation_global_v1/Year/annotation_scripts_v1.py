...
def get_unique_types(df, column):
    """ Extract unique types from a DataFrame column and sort them alphabetically. """
    return sorted(df[column].unique())  # Sorting added here
...
def apply_colors(df, color_dict):
    ...
    df['Color'] = df['Type'].apply(lambda x: color_dict.get(str(x).strip(), 'Unknown'))  # Convert x to string here and use get for safe lookup
    return df
